...
p <- 500  # number of persons and thus number of rows in data
...
data <- array(NA, c(p, n))
colnames(data)<-c("x1", "x2", "x3","x4","x5", "y1", "y2", "y3","y4","y5")
...
while(simteller<=nsim){
  ...
  warning <- any(is.na(parameterEstimates(riclpmModel.kappa.Constrainedsim, standardized = T)[c(11:20,34:64),]))
  ...
  p5 ~~ uv*q5'
    ...
    warning <- any(is.na(parameterEstimates(riclpmModel.omega.Constrainedsim, standardized = T)[c(11:20,34:64),]))
  ...
  p5 ~~ uv*q5'
      ...
      warning <- any(is.na(parameterEstimates(riclpmConstrainedsim, standardized = T)[c(11:20,34:64),]))
    ...
    S1 <- vcov(riclpmConstrainedsim)[c(11,12), c(11,12)]
    S2 <- vcov(riclpmModel.kappa.Constrainedsim)[11,11]
    S3 <- vcov(riclpmModel.omega.Constrainedsim)[11,11]
    S4 <- vcov(riclpmConstrainedsim)[12,12]
    S5 <- vcov(riclpmConstrainedsim)[11,11]
