...
p <- 200  #number of persons and thus number of rows in data
...
B2<-matrix(c(0.22, -0.07, -0.07, 0.28),byrow = T, 2) # alpha, beta, gamma, delta 
...
H0 <- "Var_kappa == 0 ; Var_omega == 0"
H1 <- "Var_kappa > 0.04 ; Var_omega > 0.04" #the specified RI-CLPM
H2 <- "Var_kappa < 0 ; Var_omega < 0" #which included equal to 0 and thus represents the CLPM
H3 <- "Var_kappa > 0.04 ; Var_omega < 0"
H3a <- "Var_kappa > 0.04 ; Var_omega == 0"
H3b <- "Var_kappa > 0.04 ; Var_omega < 0.04"
H4 <- "Var_kappa < 0 ; Var_omega > 0.04"
H4a <- "Var_kappa == 0 ; Var_omega > 0.04"
H4b <- "Var_kappa < 0.04 ; Var_omega > 0.04" 
...
while(simteller<=nsim){
  ...
  if(warning == TRUE) {
    ...
    names(unstdRIvar) <- c("Var_kappa", "Var_omega")
