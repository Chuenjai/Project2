...
H1 <- "kappaV > 0.04 ; omegaV > 0.05" #the specified RI-CLPM
H2 <- "kappaV < 0 ; omegaV < 0" #which included equal to 0 and thus represents the CLPM
H3 <- "kappaV > 0.04 ; omegaV < 0"
H3a <- "kappaV > 0.04 ; omegaV == 0"
H3b <- "kappaV > 0.04 ; omegaV < 0.05"
H4 <- "kappaV < 0 ; omegaV > 0.05"
H4a <- "kappaV == 0 ; omegaV > 0.05"
H4b <- "kappaV < 0.04 ; omegaV > 0.05" 
...
while(simteller<=nsim){
  ...
  if(warning == TRUE) {
    ...
    WarningTeller <- WarningTeller + 1
    #print(simteller)
    #print(lavInspect(riclpmConstrainedsim, "cor.lv"))
    #print(eigen(lavInspect(riclpmConstrainedsim, "cov.lv"))$values)
    #stop()
