...
varxy <- array(NA, c(p, n)) # create array with nrow=p and ncol=n to keep simulate data
...
while(simteller<=nsim){
  ...
  warning <- any(is.na(parameterEstimates(riclpmModel.kappa.Constrainedsim, standardized = T)[c(7:12,22:38),6:9]))
  ...
  warning <- any(is.na(parameterEstimates(riclpmModel.omega.Constrainedsim, standardized = T)[c(7:12,22:38),6:9]))
  ...
  warning <- any(is.na(parameterEstimates(riclpmConstrainedsim, standardized = T)[c(7:12, 15, 22:38),6:9]))
  ...
  if(warning == TRUE) {
    ...
    S1 <- vcov(riclpmConstrainedsim)[c(7,8), c(7,8)]
    S2 <- vcov(riclpmModel.kappa.Constrainedsim)[7,7]
    S3 <- vcov(riclpmModel.omega.Constrainedsim)[7,7]
    S4 <- vcov(riclpmConstrainedsim)[8,8]
    S5 <- vcov(riclpmConstrainedsim)[7,7]
    ...
    ChiBar2DiffTest_clpm_riclpm.kappa <- ChiBarSq.DiffTest(1, as.matrix(S2), Chi2_clpm, Chi2_riclpm_kappa, df_clpm, df_riclpm_kappa) # clpm and riclpm.kappa, 1= number of variance from riclpm(kappa), S = variance matrix of kappa (1 element only) from riclpm(kappa)
    ChiBar2DiffTest_clpm_riclpm.omega <- ChiBarSq.DiffTest(1, as.matrix(S3), Chi2_clpm, Chi2_riclpm_omega, df_clpm, df_riclpm_omega) # clpm and riclpm.omega, 1= number of variance from riclpm(omega), S = variance matrix of omega (1 element only) from riclpm(omega)
    ChiBar2DiffTest_riclpm_riclpm.kappa <- ChiBarSq.DiffTest(2, as.matrix(S4), Chi2_riclpm_kappa, Chi2_riclpm, df_riclpm_kappa, df_riclpm) # riclpm and riclpm.kappa, 2= number of variances from riclpm, S = variance matrix of omega (1x1) from riclpm
    ChiBar2DiffTest_riclpm_riclpm.omega <- ChiBarSq.DiffTest(2, as.matrix(S5), Chi2_riclpm_omega, Chi2_riclpm, df_riclpm_omega, df_riclpm) # riclpm and riclpm.omega, 2= number of variances from riclpm, S = variance matrix of kappa (1x1) from riclpm
